class HeatMapUrl < ActiveRecord::Base
  has_many :clicks, :class_name => "HeatMapClick"

  class << self
    def addClick(url, x, y)
      page = findByUrl url
      unless page
        page = HeatMapUrl.create({:url => url })
      end
      page.clicks.create({:x => x, :y => y})
      page
      
    end
    def create_heatmap(url)
    ...
    private
    def findByUrl( url)
      HeatMapUrl.find(:first, :conditions => ["url=?", url] )
    end
  end
end
...
module Heatmap
  ...
  DOTIMAGE = "#{RAILS_ROOT}/vendor/plugins/click_heat_map/lib/bolillamia16.png"
  COLORIMAGE ="#{RAILS_ROOT}/vendor/plugins/click_heat_map/lib/colors.png"
  OPACITY = 0.8
  DOTWIDTH = 16
